/**
 * Created by a_wav on 2017/2/3.
 */
'use strict';

function buildStyleBB(inputText) {
    ...
    if(newline == true) {
        return outputText + '\n' + buildStyleBB(inputText.substring((i+2),inputText.length));
    } else {
        return outputText;
    }
}
...
function buildStyleAA(inputText) {
    ...
    var outputText = line0+"\n"+line1+"\n"+line2+"\n"+line3+"\n"+line4+"\n"+line5;
    if(newline == true) {
        return outputText + '\n' + buildStyleAA(inputText.substring((i+2),inputText.length));
    } else {
        return outputText;
    }
}

function buildStyleCC(inputText,booleanRepeat) {
    ...
    var outputText = line0+"\n"+line1+"\n"+line2;
    if(newline == true) {
        return outputText + '\n' + buildStyleCC(inputText.substring((i+2),inputText.length));
    } else {
        return outputText;
    }
}
...
function beginGenerator(inputText, textStyle) {
    ...
    if (validChars == false) {
        throw new Error('Character "' + invalChar + '" invalid.  Only characters a-z, 0-9, and newlines (\\n) accepted.')
        return '';
    }
    if (textStyle === 1) {
        return buildStyleAA(inputText)
    }
    if (textStyle === 2) {
        return buildStyleBB(inputText)
    }
    if (textStyle === 3) {
        return buildStyleCC(inputText)
    }
    return ''
}
